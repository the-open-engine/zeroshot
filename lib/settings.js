...
let claudePathWarningShown = false;

function maybePreferLocalClaudeBinary(command, source) {
  if (command !== 'claude') {
    return command;
  }
  ...
  const localClaude = path.join(homeDir, '.local', 'bin', 'claude');
  if (!fs.existsSync(localClaude)) {
    return command;
  }

  const globalClaude = '/usr/bin/claude';
  if (!claudePathWarningShown && fs.existsSync(globalClaude)) {
    claudePathWarningShown = true;
    console.warn(
      `[settings] Found both ${localClaude} and ${globalClaude}. ` +
        `Using ${localClaude} for consistency (${source}=claude).`
    );
  }
...
const DEFAULT_SETTINGS_BASE = {
  ...
  noOutputTimeoutMs: 120000, // Fail task if no log output is produced within this window
...
function validateSetting(key, value) {
  ...
  if (
    ...
    const min =
      key === 'noOutputTimeoutMs' ? 1000 : ['backoffBaseMs', 'backoffMaxMs'].includes(key) ? 0 : 1;
    if (value < min) {
...
function getClaudeCommand() {
  ...
  const source = process.env.ZEROSHOT_CLAUDE_COMMAND ? 'env' : 'settings';
  const command = maybePreferLocalClaudeBinary(parts[0], source);
  return {
    command,
    args: parts.slice(1),
  };
