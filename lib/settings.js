...
// Claude auth functions moved to ./settings/claude-auth.js
const { ANTHROPIC_KEY_PREFIX, isValidAnthropicKey } = require('./settings/claude-auth');
...
/**
 * Validate a value is string or null
 * @param {any} value - Value to validate
 * @param {string} name - Setting name for error message
 * @returns {string|null} Error message if invalid, null if valid
 */
function validateStringOrNull(value, name) {
...
/**
 * Validate providerSettings structure
 * @returns {string|null} Error message if invalid, null if valid
 */
function validateProviderSettings(value) {
  ...
  for (const [provider, settings] of Object.entries(normalizedSettings || {})) {
    ...
    // Claude-specific auth validation
    if (provider === 'claude') {
      // Validate string-or-null fields
      for (const field of ['anthropicApiKey', 'bedrockApiKey', 'bedrockRegion']) {
        const err = validateStringOrNull(settings[field], `providerSettings.claude.${field}`);
        if (err) return err;
      }
