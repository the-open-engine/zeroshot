 ...
 * Automatically falls back to bunx if default claude command not found
 * @returns {{ command: string, args: string[] }}
 */
function getClaudeCommand() {
  const settings = loadSettings();
  let raw = process.env.ZEROSHOT_CLAUDE_COMMAND || settings.claudeCommand || 'claude';

  if (raw === 'claude') {
    for (const cmd of ['claude', 'bunx @anthropic-ai/claude-code']) {
      try {
