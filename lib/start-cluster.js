...
function buildStartOptions({
  ...
  const plan = resolveRunPlan(mergedOptions);
  return {
    clusterId,
    cwd: resolveTargetCwd(),
    isolation: anyTruthy(
      plan.isolation === 'docker',
    ...
    worktree: anyTruthy(plan.isolation === 'worktree', process.env.ZEROSHOT_WORKTREE === '1'),
    autoPr: anyTruthy(plan.delivery !== 'none', process.env.ZEROSHOT_PR === '1'),
    autoMerge: anyTruthy(plan.autoMerge, process.env.ZEROSHOT_MERGE === '1'),
