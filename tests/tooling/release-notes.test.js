...
describe('deterministic release notes', () => {
  ...
  it('uses leading prose when a squash body has no Summary heading', () => {
    const body = [
    ...
    assert.equal(summaryFromBody(body), 'Upgrade the parser and bound aliases.');
  });
  ...
  it('uses the preceding canonical tag and immutable first-parent range', () => {
    ...
    try {
      ...
      git(
        ...
        '--allow-empty',
        '-m',
        'chore: establish release baseline (#40)',
        '-m',
