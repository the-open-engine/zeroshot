...
describe('Release-note publication contract', () => {
  it('generates one immutable notes artifact and republishes it exactly', () => {
    ...
    const tooling = planSteps.get('Checkout release-note tooling');
    const source = planSteps.get('Checkout exact Zeroshot release source');
    const generate = planSteps.get('Generate deterministic release notes');
    ...
    assert.match(generate.run, /node release-tooling\/scripts\/release-notes\.js/);
    ...
    assert.match(publish.run, /published_notes=.*gh release view/);
    assert.match(publish.run, /"\$published_notes" == "\$expected_notes"/);
