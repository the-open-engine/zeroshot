...
function startIsolatedCluster(config, text) {
...
describe('Orchestrator Isolation Mode Integration', function () {
  ...
  defineContainerLifecycleTests();
  defineAgentExecutionTests();
  defineResumeCapabilityTests();
  defineGitIsolationTests();
});

function defineContainerLifecycleTests() {
  describe('Container Lifecycle', function () {
    it('should create container with correct mounts and state', async function () {
      await runContainerCreationTest();
    });

    it('should preserve workspace on stop for resume capability', async function () {
      await runPreserveWorkspaceOnStopTest();
    });

    it('should fully clean up workspace and container on kill', async function () {
      await runFullCleanupOnKillTest();
    });

    it('should force-remove container on kill', async function () {
      await runForceRemoveContainerTest();
    });
  });
}

async function runContainerCreationTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(simpleConfig, 'Test task');
  ...
  const wsExists = execSync(`docker exec ${containerId} test -d /workspace && echo yes || echo no`)
    .toString()
    .trim();
  assert.strictEqual(wsExists, 'yes', 'Workspace should be mounted');

  // VERIFY: Git repo with feature branch
  const branch = execSync(`docker exec ${containerId} git branch --show-current`).toString().trim();
...
async function runFullCleanupOnKillTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(simpleConfig, 'Test kill cleanup');
...
async function runForceRemoveContainerTest() {
  mockRunner.when('worker').delays(60000, '{"done": true}'); // Long delay

  const result = await startIsolatedCluster(simpleConfig, 'Test kill');
...
function defineAgentExecutionTests() {
...
async function runWorkerValidatorFlowTest() {
  ...
  const result = await startIsolatedCluster(workerValidatorConfig, 'Test multi-agent');
...
async function runIsolationMessageOrderTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(simpleConfig, 'Test message order');
...
async function runResumeContainerRecreationTest() {
  ...
  assert.notStrictEqual(newContainerId, oldContainerId, 'New container ID should differ from old');
...
async function runResumeAgentUpdateTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(resumeTestConfig, 'Test agent update');
...
async function runResumeNotResumableTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(resumeTestConfig, 'Test fail');
...
function defineGitIsolationTests() {
  describe('Git Isolation', function () {
    it('should create fresh git repo with single initial commit', async function () {
      await runGitIsolationInitialCommitTest();
    });

    it('should create feature branch with clusterId', async function () {
      await runGitIsolationBranchTest();
    });
  });
}

async function runGitIsolationInitialCommitTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(simpleConfig, 'Test git isolation');
  ...
  const commitMsg = execSync(`docker exec ${containerId} git log -1 --format=%s`).toString().trim();
  assert(
...
async function runGitIsolationBranchTest() {
  mockRunner.when('worker').returns('{"done": true}');

  const result = await startIsolatedCluster(simpleConfig, 'Test branch');
