...
5. creates or verifies the GitHub Release with notes generated by the exact release source from its
   immutable squash commits, then uploads exact artifacts, including the release-note Markdown;
...
GitHub Release body from `zeroshot-release-notes-vX.Y.Z.md` and verifies it byte-for-byte. When the
exact source contains the generator, it must reproduce that asset before recovery continues.
