...
fn lifecycle_publish_then_materialize_on_start() {
    ...
    // cycle 1 → fence 1
    match publish_cycle(&tree_a, &s, &ls, &clock, &lin).unwrap() {
    ...
    let head2 = match publish_cycle(&tree_a, &s, &ls, &clock, &lin).unwrap() {
...
fn mf3_dedup_reuse_survives_gc() {
    ...
    // cycle 1
    let h1 = match publish_cycle(&tree, &s, &ls, &clock, &lin).unwrap() {
    ...
    let h2 = match publish_cycle(&tree, &s, &ls, &clock, &lin).unwrap() {
        CycleOutcome::Advanced(h) => {
            assert_eq!(h.fence, Fence(2));
            h
        }
...
fn stale_fence_surfaces_as_non_fatal_fenced() {
    ...
    for attempt in 0..16u32 {
        ...
        match publish_cycle(&seed_tree, &*s, &ls, &*clock, &lin).unwrap() {
            CycleOutcome::Advanced(h) => assert_eq!(h.fence, Fence(1)),
        ...
        for t in [tree_a.clone(), tree_b.clone()] {
            ...
            handles.push(std::thread::spawn(move || {
                ...
                publish_cycle(&t, &*s, &ls, &*clock, &lin)
            }));
        }
