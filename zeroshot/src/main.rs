...
use zeroshot_engine::native_v2_cli::{
    ...
    try_execute_native_v2_static, AcpServeError, CtrlCDetachSignal, NativeV2CliBackend,
...
async fn dispatch(command: NativeV2CliCommand) -> Result<(), ProcessError> {
    match command {
        NativeV2CliCommand::Acp { profile } => {
            serve_local_acp(profile).await.map_err(ProcessError::from)
        }
        NativeV2CliCommand::Ui { listen, target } => serve_ui(listen, target).await,
        NativeV2CliCommand::TargetServe(config) => serve_direct_target(config)
            .await
...
async fn run_public_command(command: NativeV2CliCommand) -> Result<(), ProcessError> {
    ...
    execute_public_command(command, &mut output).await
}
...
async fn execute_public_command(
    ...
    if try_execute_native_v2_static(&command, output)?.is_some() {
        return Ok(());
    }
    if try_execute_native_v2_preflight(&command, output)
        .await?
        .is_some()
    {
        return Ok(());
    ...
    if is_local_command(&command) {
        return run_local_command(command, &mut detach, output).await;
    }
    run_named_target_command(command, &mut detach, output).await
}
...
async fn run_named_target_command(
    ...
    let connector = NativeV2TargetConnector::new(registry, authority, TargetOecpWebSocketDialer);
    let backend = NamedTargetCliBackend::new(connector);
    execute_backend_command(command, &backend, detach, output).await
}
...
async fn execute_backend_command<B: NativeV2CliBackend>(
    command: NativeV2CliCommand,
    backend: &B,
    detach: &mut CtrlCDetachSignal,
    output: &mut impl Write,
...
fn is_local_command(command: &NativeV2CliCommand) -> bool {
    match command {
        ...
        command => run_operation_is_local(command)
            .or_else(|| management_operation_is_local(command))
            .unwrap_or(false),
    }
}

fn run_operation_is_local(command: &NativeV2CliCommand) -> Option<bool> {
    let local = match command {
        ...
        NativeV2CliCommand::Resume(command) => command.run.target.is_none(),
        NativeV2CliCommand::Checkpoints(command) => command.run.target.is_none(),
...
fn management_operation_is_local(command: &NativeV2CliCommand) -> Option<bool> {
    let local = match command {
...
async fn serve_ui(
    ...
    serve_ui_with_registry_path(listen, target, default_target_registry_path).await
}

async fn serve_ui_with_registry_path(
    listen: std::net::SocketAddr,
    target: Option<String>,
    registry_path: impl FnOnce() -> Result<PathBuf, TargetConnectorError>,
) -> Result<(), ProcessError> {
    #[cfg(feature = "ui")]
    {
        let target = target
            .map(|name| resolve_ui_target(name, registry_path))
            .transpose()?;
    ...
    #[cfg(not(feature = "ui"))]
    {
        let _ = (listen, target, registry_path);
        let message = concat!(
...
fn resolve_ui_target(
    ...
    let registry = FileTargetRegistry::new(registry_path()?);
